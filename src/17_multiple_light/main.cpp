...
public:
    ...
    virtual void startup()
    {
        ...
        fragmentShaderPath = rootDir + std::string("/src/shader/16_light_casters.fs");
...
//        objectShader->setVec3("light.position", lightPos.x, lightPos.y, lightPos.z);
        ...
        objectShader->setFloat("light.linear", 0.0014f);
        objectShader->setFloat("light.quadratic", 0.000007f);
        // 聚光
        objectShader->setVec3("light.position", camera.position_.x, camera.position_.y, camera.position_.z);
        objectShader->setVec3("light.direction", camera.front_.x, camera.front_.y, camera.front_.z);
        objectShader->setFloat("light.cutOff", cos(RADIANS(12.5f)));
        objectShader->setFloat("light.outCutOff", cos(RADIANS(17.5f)));
        ...
        for (int i = 0; i < 10; ++i) {
            modelMat = TransMat4::Translation(cubePositions[i].x, cubePositions[i].y, cubePositions[i].z);
            rotateMat = TransMat4::Rotation(1.0f, 1.0f, 1.0f, 20*i+(float)currentTime);
